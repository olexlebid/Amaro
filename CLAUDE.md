...
- **Статус проєкту: в розробці.** Поки статус не зміниться на «в продакшені» — без нових гілок і PR, усе комітиться напряму в `main` невеликими осмисленими комітами (див. §11.2).
...
| `news` | title, date, category, excerpt, cover, coverAlt, body (Markdoc) — пізніше автоматично з Instagram через Make |
...
2. Статус проєкту «в розробці» (§1) → **без нових гілок**, коміти йдуть напряму в `main`, невеликі й осмислені. Коли статус зміниться на «в продакшені» — повертаємось до гілки на кожну задачу (з PR і preview URL, §13).
...
| [19.09.2026] | GSAP + ScrollTrigger (нова залежність) для Gallery pin+horizontal-scrub | Referenced papertiger.com/agency's "about-team" section. Перша scroll-driven анімація в проєкті, що вимагала pin+scrub — vanilla CSS scroll-timeline недостатньо надійний крос-браузерно (Safari). Клієнт схвалив бібліотеку; на цьому етапі Lenis (site-wide inertia scroll) свідомо НЕ додано — замість цього `scrub: 1` (числове значення) давало той самий ефект "згладженого наздоганяння", але лише в межах секції Gallery |
| [20.09.2026] | Lenis (нова залежність) — тепер site-wide inertia scroll, замінює рішення від 19.09 | Клієнт попросив той самий плавний скрол, що на papertiger.com/agency, для всього сайту, не лише Gallery. Підключено в `SmoothScroll.astro` (нова секція в `BaseLayout.astro`), синхронізовано з GSAP `ScrollTrigger` через `lenis.on('scroll', ScrollTrigger.update)` + `gsap.ticker`. Gallery.astro: `scrub: 1` → `scrub: true` — з Lenis подвійне згладжування (власний scrub-лаг поверх уже згладженого Lenis-скролу) відчувалося занадто "тягучим". Вимикається повністю під `prefers-reduced-motion` (нативний скрол лишається) |
| [20.09.2026] | ProgramsCarousel.astro переведено на GSAP ScrollTrigger + Lenis (той самий стек, що й Gallery), замінивши власний rAF/`window.scroll`-скрипт | Performance-аудит (Chrome trace) показав три незалежні scroll-реактивні системи одночасно (Lenis, Gallery's ScrollTrigger, і hand-rolled pin у ProgramsCarousel) — тепер один двигун на сторінку. `position: sticky` на `.programs__viewport` прибрано (GSAP пінить `.programs__pin` через `pinType: "transform"`, як і Gallery — той самий CLS-фікс). Власний `getMode/applyMode/AbortController` (stacked/native/pinned) замінено на єдиний `ScrollTrigger.matchMedia` з трьома ключами — прибрало другий, вручну синхронізований `@media`-гейт, який уже раз ламався (описано в коментарі коду як "disappearing section" bug). Уся математика центрування/паралаксу (`TRACK_BOX_EXTEND`, `HEADING_OVERHANG`, `HEADING_PARALLAX_*`) залишена без змін |
| [20.09.2026] | GSAP SplitText (новий плагін з уже підключеного `gsap`, окремо нічого не встановлювали — з версії, що в проєкті, він безкоштовний) для word-by-word "fade up" reveal | Клієнт попросив однаковий ефект появи тексту в CountdownCta.astro (текст+кнопка після завершення утримання) і в Hero.astro (заголовок H1). Hero H1 переведено з ручного blur-text reveal (рішення 16.09.2026: CSS `@keyframes` + `animation-delay` по `data-word`-спанах, свідомо без бібліотек) на `SplitText.create(heading, {type:"words", ignore:"[data-since]"})` + `gsap.timeline()` — код і розмітка спростились (ручні `data-word`-спани й keyframes прибрані), клієнт відзначив як потенційно продуктивніше. `est. 2006` виключено з split через `ignore` і анімується окремим кроком тому ж таймлайні (`"-=0.3"` внахлест). Тригер (очікування зникнення `html.has-preloader` через `MutationObserver`) і no-JS/`prefers-reduced-motion`-fallback (звичайний видимий текст) — без змін |
| [22.09.2026] | Статус проєкту → «в розробці»: без гілок/PR, усі коміти напряму в `main` | Проєкт ще не в продакшені, окремого клієнта/команди на review немає — гілки й PR тільки сповільнюють ітерації. Правило §11.2 умовне: повертаємось до гілки на задачу, коли статус зміниться на «в продакшені» |
| [22.09.2026] | Блог (`news`): Markdoc (`.mdoc`) замість JSON чи "чистого" MDX, `@astrojs/markdoc` (нова легка інтеграція, без SSR) | Довгий текст статті з форматуванням незручно тримати в JSON-полі (як `courses`). Markdoc обраний конкретно тому, що поле-редактор Keystatic (`fields.document()`) серіалізується саме в Markdoc — коли пізніше даємо контент-крієтору доступ через Keystatic-адмінку, вона просто відкриє наявні `.mdoc`-файли в `src/content/news/` без конвертації контенту. До того часу статті пишу/редагую сам через Claude. Додано `/blog` (hub+listing, спрощено проти referenced breef.com/breefingroom — без окремого /articles рівня, обсяг контенту не виправдовує) і `/blog/[slug]`; Figma-фрейму для блогу ще нема — верстка функціональна на токенах, чекає дизайну |
| [23.09.2026] | `news`-схема розширена необов'язковими полями `metaTitle`, `metaDescription`, `author`, `featured` | Звірено з прикладом полів Webflow-блогу (Breef.com CSV). Webflow-автополя (Created/Updated/Published On, Draft/Archived, Collection/Locale/Item ID) свідомо не додані — не мають сенсу в git-based CMS без окремого draft-стану (файл у репо = стаття опублікована), `date` лишається єдиним авторським полем дати. `Time to read` лишається обчислюваним з тексту, не окремим полем. `author` — просте текстове поле (не `reference()` на `team`, бо `team`/`team-bios` ще не існують як колекції). `featured` сортує статтю першою на `/blog` + бейдж "Empfohlen" на картці (`NewsCard.astro`, той самий пілюля-прийом, що й `.hero__adtv-badge` в Hero.astro) — мінімальна реалізація без Figma-дизайну блогу |
